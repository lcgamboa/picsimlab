...
// Font scale helper: scales a design-baseline pixel value by the font scale factor
#define S(v) ((int)((v) * fscale))

// Implementation

void CPWindow1::_EvOnCreate(CControl *control) {
  font = new lxFont(10, lxFONTFAMILY_TELETYPE, lxFONTSTYLE_NORMAL,
                    lxFONTWEIGHT_BOLD);
  if (Application->Aargc == 2) {
    port = atoi(Application->Aargv[1]);
  }
}
...
void CPWindow1::_EvOnHide(CControl *control) {
...
void CPWindow1::_EvOnDestroy(CControl *control) { delete font; }

int CPWindow1::PointInside(int i, int x_, int y_) {
  ...
  } else // pincount > 40
  {
    if (i < pincount / 4) {
      ...
      x = S(300);
      y = (i - (pincount / 4)) * S(15) + S(20);
    } else if (i < pincount * 3 / 4) {
      x = S(600);
      y = (i - (pincount * 2 / 4)) * S(15) + S(20);
    } else {
      y = (i - (pincount * 3 / 4)) * S(15) + S(20);
      x = S(900);
    }
  }
...
void CPWindow1::draw1_EvMouseButtonPress(CControl *control, const uint button,
  ...
  for (int i = 0; i < pincount; i++) {
    if (PointInside(i, (int)(fx - offsetx), (int)(fy - offsety)) == 1) {
      ...
      if ((pins[i].type == 'D') && (pins[i].dir == 'I')) {
        ...
        float val = (fx - offsetx - S(185) - x) / (S(52) / 5.0f);
        if (val > 5.0)
          val = 5.0;
...
void CPWindow1::draw1_EvMouseButtonRelease(CControl *control, const uint button,
                                           const uint x, const uint y,
                                           const uint state) {
  // float  fx = x / scale;
  // float  fy = y / scale;
...
void CPWindow1::draw1_EvMouseMove(CControl *control, const uint button,
                                  const uint x, const uint y,
                                  const uint state) {
  float fx = x / scale;
  float fy = y / scale;
...
void CPWindow1::menu1_File_Exit_EvMenuActive(CControl *control) { WDestroy(); }

void CPWindow1::menu1_Help_Contents_EvMenuActive(CControl *control) {
  lxString stemp;
  stemp.Printf(
      lxT("https://lcgamboa.github.io/picsimlab_docs/%s/PinViewer.html"),
      lxT(_VERSION_));
  lxLaunchDefaultBrowser(stemp);
}

void CPWindow1::menu1_Help_About_EvMenuActive(CControl *control) {
  lxString stemp;
...
void CPWindow1::timer1_EvOnTime(CControl *control) {
  ...
  // Compute font scale factor from actual rendered font metrics.
  // The layout was designed for a 10pt monospace font (~12px tall).
  {
    wxCoord tw, th;
    draw1.Canvas.GetDC()->GetTextExtent(wxT("X"), &tw, &th);
    fscale = (float)th / 12.0f;
  ...
  if (connected) {
    ...
    if (pinsc != pincount) {
      ...
      if (pincount <= 8) {
        SetHeight(pincount * S(15) + S(130));
        SetWidth(S(280));
      } else if (pincount <= 40) {
        SetHeight((pincount / 2) * S(15) + S(130));
        SetWidth(S(580));
      } else {
        SetHeight((pincount / 4) * S(15) + S(130));
        SetWidth(S(1170));
      }
      scale = 1.0;
      offsetx = 0;
    ...
    while (pinsc) {
      ...
      case 'D':
        ...
        draw1.Canvas.Rectangle(1, S(8) + offsetx + x, y - S(1) + offsety, S(238), S(14));
        ...
        draw1.Canvas.Rectangle(1, S(98) + offsetx + x, y + S(1) + offsety, S(142), S(10));
        ...
        draw1.Canvas.Rectangle(1, S(134) + offsetx + x, y + S(2) + offsety, S(30), S(8));

        draw1.Canvas.SetFgColor(255, 255, 255);
        draw1.Canvas.Text(line, S(10) + offsetx + x, y - S(1) + offsety);

        // togle and mean
        if (dir[0] == 'I') {
          draw1.Canvas.SetColor(200, 100, 30);
          draw1.Canvas.Rectangle(1, S(185) + offsetx + x, y + S(2) + offsety, S(52), S(8));
          draw1.Canvas.SetFgColor(255, 255, 255);
          draw1.Canvas.Text("Toggle", S(187) + offsetx + x, y - S(1) + offsety);
        } else {
          sscanf(mvalue, "%3d", &mv);
          draw1.Canvas.SetColor(200, 200, 200);
          draw1.Canvas.Rectangle(1, S(185) + offsetx + x, y + S(2) + offsety, S(52), S(8));
          draw1.Canvas.SetColor(0, 0, 200);
          draw1.Canvas.Rectangle(1, S(186) + offsetx + x, y + S(3) + offsety,
                                 (int)(mv / 4 * fscale), S(6));
        }
...
void CPWindow1::draw1_EvMouseWheel(CControl *control, const int rotation) {
  if (rotation > 0) {
    scale += 0.1;
    if (scale > 2)
      scale = 2;
...
int CPWindow1::send_cmd(const char *message) {
  // printf ("sending '%s'\n", message);
