...
sed -i 's/Depends: gtkwave, cutecom, gedit, pulseview,/Depends: gpsim,libgcrypt20, /g' debian/control
...
if [[ -n "$1" ]]; then
	mv picsimlab_${VERSION}-1_amd64.deb  ../${DESTDIR}/picsimlab_NOGUI_${VERSION}_experimetal_${lversion}_amd64.deb
else
	mv picsimlab_${VERSION}-1_amd64.deb  ../${DESTDIR}/picsimlab_NOGUI_${VERSION}_${lversion}_amd64.deb
fi
