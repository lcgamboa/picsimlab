...
unsigned short eth_w5500_io(eth_w5500_t* eth, unsigned char mosi, unsigned char sclk, unsigned char scs,
                            unsigned char rst) {
                    ...
                    if (RWB)  // write
                    {
                        switch (BSB) {
                                            ...
                                            if (eth->Socket[n][Sn_CR]) {
                                                ...
                                                switch (eth->Socket[n][Sn_CR]) {
                                                    case OPEN:
                                                        ...
                                                        switch (eth->Socket[n][Sn_MR] & 0x0F) {
                                                            ...
                                                            case Sn_MR_TCP:
                                                                ...
                                                                if ((eth->sockfd[n] = socket(PF_INET, SOCK_STREAM, 0)) <
                                                                    0) {
                                                            ...
                                                            case Sn_MR_UDP:
                                                                ...
                                                                if ((eth->sockfd[n] = socket(AF_INET, SOCK_DGRAM, 0)) <
                                                                    0) {
                                                                    printf("eth_w5500: socket creation failed\n");
                                                                    eth->sockfd[n] = INVALID_SOCKET_VALUE;
                                                                    eth->Socket[n][Sn_SR] = SOCK_CLOSED;
                                                        ...
                                                        writeWord(eth->Socket[n], Sn_RX_RD0, 0);
                                                        writeWord(eth->Socket[n], Sn_RX_WR0, 0);
                                                        writeWord(eth->Socket[n], Sn_TX_FSR0,
                                                                  eth->Socket[n][Sn_TXBUF_SIZE] * 1024);
                                                        writeWord(eth->Socket[n], Sn_RX_RSR0, 0);
                                                        eth->status[n] = 0;
                                                        setnblock(eth->sockfd[n]);
                                                        break;
                                                    case LISTEN:
                                                        if ((eth->Socket[n][Sn_MR] & 0x0F) != Sn_MR_TCP)
                                                            break;
                                                        ...
                                                        if (eth->Socket[n][Sn_SR] != SOCK_INIT)
                                                            break;
                                                        ...
                                                        if (eth->listenfd[eth->listenfd_map[n]] ==
                                                            ...
                                                            serv.sin_addr.s_addr = htonl(INADDR_ANY);
                                                            serv.sin_port = htons(skt_port);
                                                            if (bind(eth->listenfd[eth->listenfd_map[n]],
                                                            ...
                                                            if (listen(eth->listenfd[eth->listenfd_map[n]],
                                                                       SOMAXCONN)) {
                                                    ...
                                                    case CONNECT:
                                                        ...
                                                        conn_timeout[n] = 0;
                                                        sprintf(skt_addr, "%i.%i.%i.%i", eth->Socket[n][Sn_DIPR0],
                                                                eth->Socket[n][Sn_DIPR1], eth->Socket[n][Sn_DIPR2],
                                                                eth->Socket[n][Sn_DIPR3]);
                                                        dsprintf("Socket %i try to connect to %s:%i\n", n, skt_addr,
                                                                 readWord(eth->Socket[n], Sn_DPORT0));
                                                    ...
                                                    case SEND:
                                                        ...
                                                        switch (eth->Socket[n][Sn_MR] & 0x0F) {
                                                            case Sn_MR_TCP:
                                                                ...
                                                                if (eth->Socket[n][Sn_SR] != SOCK_ESTABLISHED)
                                                                    break;
...
#endif
                                                                if ((s = send(eth->sockfd[n],
                                                                              (const char*)&eth->TX_Mem[eth->TX_ptr[n]],
                                                                              size, MSG_NOSIGNAL)) < 0) {
                                                            ...
                                                            case Sn_MR_UDP:
                                                                ...
                                                                sprintf(
                                                                    skt_addr, "%i.%i.%i.%i", eth->Socket[n][Sn_DIPR0],
                                                                    eth->Socket[n][Sn_DIPR1], eth->Socket[n][Sn_DIPR2],
                                                                    eth->Socket[n][Sn_DIPR3]);
                                                                skt_port = readWord(eth->Socket[n], Sn_DPORT0);
                                                                            ...
                                                                            "255.255.255.255"))  // fake broadcast
                                                                {
                                                                    ...
                                                                    sendto(eth->sockfd[n],
                                                                           (const char*)&eth->TX_Mem[eth->TX_ptr[n]],
                                                                           size, 0 /*MSG_DONTWAIT MSG_CONFIRM*/,
                                                    ...
                                                    case RECV:
                                                        dsprintf("Socket %i Receive CMD  (%i)\n", n,
                                                                 readWord(eth->Socket[n], Sn_RX_RSR0));
                                                        // dsprintf ("RX_WR0=%i   RX_RD0=%i  \n", readWord
                                                        // (eth->Socket[n], Sn_RX_WR0), readWord (eth->Socket[n],
                                                        // Sn_RX_RD0));
                            ...
                            case B_SCK7TX:
                                ...
                                // readWord (eth->Socket[n], Sn_TX_WR0));
                        ...
                        dprintf("dsr %03i w5500= 0x%04X  write data=0x%02X (%i)\n", eth->bb_spi.byte << 3,
                                eth->bb_spi.insr, eth->bb_spi.insr & 0x00FF, offset);
                    } else  // read
                    {
                        switch (BSB) {
                            case B_COMMON:
                        ...
                        dprintf("dsr %03i w5500= 0x%04X  read data=0x%02X (%i)\n", eth->bb_spi.byte << 3,
                                eth->bb_spi.insr, eth->bb_spi.outsr, offset + 1);
                    }
                    break;
            }
            break;
