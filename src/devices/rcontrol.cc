...
static void ProcessInput(const char *msg, input_t *Input, int *ret) {
  ...
  if ((Input->name[0] == 'V') && (Input->name[1] == 'S')) {
    short temp = ((*((unsigned char *)Input->status)) << 8) |
                 (*(((unsigned char *)(Input->status)) + 1));
    stemp.Printf("%s %s= %i\r\n", msg, Input->name, temp);
    ...
    if (strchr(buffer, '\n')) {
      ...
      case 'd':
        if (strstr(cmd, "dumpr")) {
          ...
          {
            for (unsigned int i = addr;
                 (i < (addr + size)) && i < Board->DBGGetRAMSize(); i += 16) {
              snprintf(lstemp, 100, "%04X: ", i);
              ret += sendtext(lstemp);

              for (unsigned int j = 0; (j < 16) && (j < size - (i - addr)) &&
                                       (i + j) < Board->DBGGetRAMSize();
                   j++) {
                snprintf(lstemp, 100, "%02X ", Board->DBGGetRAM_p()[j + i]);
                ret += sendtext(lstemp);
              }
          ...
          {
            for (unsigned int i = addr;
                 (i < (addr + size)) && i < Board->DBGGetEEPROM_Size();
                 i += 16) {
              snprintf(lstemp, 100, "%04X: ", i);
              ret += sendtext(lstemp);

              for (unsigned int j = 0; (j < 16) && (j < size - (i - addr)) &&
                                       (i + j) < Board->DBGGetEEPROM_Size();
                   j++) {
                snprintf(lstemp, 100, "%02X ", Board->DBGGetEEPROM_p()[j + i]);
                ret += sendtext(lstemp);
              }
          ...
          {
            for (unsigned int i = addr;
                 (i < (addr + size)) && i < Board->DBGGetROMSize(); i += 16) {
              snprintf(lstemp, 100, "%04X: ", i);
              ret += sendtext(lstemp);

              for (unsigned int j = 0; (j < 16) && (j < size - (i - addr)) &&
                                       (i + j) < Board->DBGGetROMSize();
                   j++) {
                snprintf(lstemp, 100, "%02X ", Board->DBGGetROM_p()[j + i]);
                ret += sendtext(lstemp);
              }
      ...
      case 'g':
        if (!strncmp(cmd, "get ", 4)) {
          ...
          if ((ptr = strstr(cmd, " board.in["))) {
            ...
            snprintf(lstemp, 100,
                     "pin[%02i] %c %c %i %03i %5.3f \"%-8s\" \r\nOk\r\n>", pin,
                     pintypetoletter(pins[pin - 1].ptype),
                     (pins[pin - 1].dir == PD_IN) ? 'I' : 'O',
                     pins[pin - 1].value, (int)(pins[pin - 1].oavalue - 55),
                     pins[pin - 1].avalue,
      ...
      case 'h':
        if (!strcmp(cmd, "help")) {
          ...
          ret +=
              sendtext("  pins         - show pins directions and values\r\n");
        ...
        if (!strcmp(cmd, "pins")) {
          ...
          for (i = 0; i < Board->MGetPinCount(); i++) {
            snprintf(lstemp, 100,
                     ...
                     (pins[i].dir == PD_IN) ? 'I' : 'O', pins[i].value,
        ...
        if (!strncmp(cmd, "set ", 4)) {
          ...
          if (strstr(cmd + 3, "stop")) {
            ...
            if (Window1.GetSimulationRun()) {
              ret = sendtext(
                  lxString().Format("Simulation running %5.2fx\r\nOk\r\n>",
