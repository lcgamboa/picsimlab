        ...
        if (!output[i].r)  // if output shape is a rectangle
        {
            ...
            switch (output[i].id)  // search for color of output
            {
                ...
                case O_D4:  // Red using pin 2 mean  value (RA5)
                    PICSimLab.CanvasCmd({.cmd = CC_SETCOLOR, .SetColor{0, (unsigned int)pic.pins[1].oavalue, 0}});
                    break;
                case O_D5:  // Red using pin 18 mean value (RA1)
                    PICSimLab.CanvasCmd({.cmd = CC_SETCOLOR, .SetColor{0, (unsigned int)pic.pins[17].oavalue, 0}});
                    break;
                case O_D6:  // Red using pin 17 mean value (RA2)
                    PICSimLab.CanvasCmd({.cmd = CC_SETCOLOR, .SetColor{0, (unsigned int)pic.pins[16].oavalue, 0}});
                    break;
                case O_D7:  // Red using pin 5 mean
                            // value (RC5)
                    PICSimLab.CanvasCmd({.cmd = CC_SETCOLOR, .SetColor{0, (unsigned int)pic.pins[4].oavalue, 0}});
                    break;
                case O_JMP:
