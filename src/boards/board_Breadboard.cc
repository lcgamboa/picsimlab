...
void cboard_Breadboard::EvKeyPress(unsigned int key, unsigned int mask) {}

// Event on the board

void cboard_Breadboard::EvKeyRelease(unsigned int key, unsigned int mask) {}

// Event on the board

void cboard_Breadboard::EvMouseButtonPress(unsigned int button, unsigned int x, unsigned int y, unsigned int state) {
    int i;
...
void cboard_Breadboard::EvMouseButtonRelease(unsigned int button, unsigned int x, unsigned int y, unsigned int state) {
    int i;
                        ...
                        PICSimLab.CanvasCmd(
                            {.cmd = CC_RECTANGLE,
                             .Rectangle{1, output[i].x1, output[i].y1, ((output[i].x2 - output[i].x1) * 0.65f),
                                        output[i].y2 - output[i].y1}});
                        PICSimLab.CanvasCmd({.cmd = CC_SETBGCOLOR, .SetBgColor{220, 220, 0}});
...
void cboard_Breadboard::SetScale(double scale) {
    ...
    if (MGetPinCount()) {
        ...
        bmp = PICSimLab.CanvasCmd(
            {.cmd = CC_LOADIMAGE,
             .LoadImage{(PICSimLab.GetSharePath() + "boards/Common/ic40.svg").c_str(), Scale, 1, 0}});
    }
    if (micbmp >= 0)
        PICSimLab.CanvasCmd({.cmd = CC_FREEBITMAP, .FreeBitmap{micbmp}});
    micbmp = bmp;
