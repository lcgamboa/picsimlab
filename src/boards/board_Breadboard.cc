...
unsigned short cboard_Breadboard::get_in_id(char *name) {
...
unsigned short cboard_Breadboard::get_out_id(char *name) {
...
cboard_Breadboard::cboard_Breadboard(void)
  ...
  image.LoadFile(
      lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/ic40.svg")), 0,
      Scale, Scale, 1);
    ...
    if (serialfd != INVALID_HANDLE_VALUE)
      ...
      Window1.statusbar1.SetField(
          2, lxT("Serial: ") + lxString::FromAscii(SERIALDEVICE) + lxT(":") +
                 itoa(serialbaud) + lxT("(") +
                 lxString().Format(
                     "%4.1f", fabs((100.0 * serialexbaud - 100.0 * serialbaud) /
                                   serialexbaud)) +
                 lxT("%)"));
    else
      Window1.statusbar1.SetField(2, lxT("Serial: ") +
...
void cboard_Breadboard::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("Breadboard_clock"),
...
void cboard_Breadboard::EvMouseButtonPress(uint button, uint x, uint y,
                                           uint state) {
  int i;
...
void cboard_Breadboard::EvMouseButtonRelease(uint button, uint x, uint y,
                                             uint state) {
  int i;
  ...
  for (i = 0; i < outputc; i++) // run over all outputs
  {
    if (output[i].update) // only if need update
    {
      ...
      case O_LPWR: // Blue using mcupwr value
        ...
        break;
      case O_MP:
        font.SetPointSize(((MGetPinCount() >= 44) || (MGetPinCount() <= 8))
                              ? 6
        ...
        draw->Canvas.PutBitmap(micbmp, output[i].x1 * Scale,
                               output[i].y1 * Scale);
        ...
        draw->Canvas.TextOnRect(Proc, rec,
...
int cboard_Breadboard::MInit(const char *processor, const char *fname,
                             float freq) {
  int ret = 0;
  ...
  if (!image.LoadFile(lxGetLocalFile(Window1.GetSharePath() +
                                     lxT("boards/Common/ic") +
                                     itoa(MGetPinCount()) + lxT(".svg")),
                      0, Scale, Scale, 1)) {
    image.LoadFile(
        lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/ic6.svg")),
        0, Scale, Scale, 1);
    printf("picsimlab: IC package with %i pins not found!\n", MGetPinCount());
    printf("picsimlab: %s not found!\n",
           (const char *)(Window1.GetSharePath() + lxT("boards/Common/ic") +
                          itoa(MGetPinCount()) + lxT(".svg"))
               .c_str());
  }
...
void cboard_Breadboard::SetScale(double scale) {
  ...
  if (MGetPinCount()) {
    ...
    image.LoadFile(
        lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/ic40.svg")),
        0, Scale, Scale, 1);
  }
  if (micbmp)
    delete micbmp;
...
void cboard_Breadboard::EndServers(void) { mplabxd_server_end(); }
