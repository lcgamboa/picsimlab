...
unsigned short cboard_Curiosity::get_in_id(char *name) {
...
unsigned short cboard_Curiosity::get_out_id(char *name) {
  if (strcmp(name, "JP_1") == 0)
    return O_JMP;
...
void cboard_Curiosity::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("Curiosity_clock"),
...
void cboard_Curiosity::EvMouseButtonPress(uint button, uint x, uint y,
                                          uint state) {
  int i;
...
void cboard_Curiosity::EvMouseButtonRelease(uint button, uint x, uint y,
                                            uint state) {
  int i;
