...
typedef struct {
  uint32_t
      len; /**< How many points are taken (points[0] .. points[len - 1]). */
...
static uint8_t avr_usi_read(struct avr_t *avr, avr_io_addr_t addr,
                            void *param) {
...
static void avr_usi_write(struct avr_t *avr, avr_io_addr_t addr, uint8_t v,
                          void *param) {
...
static const unsigned char AVR_PORTS[12] = {'A', 'B', 'C', 'D', 'E', 'F',
                                            'G', 'H', 'I', 'J', 'K', 'L'};
...
int bsim_simavr::MInit(const char *processor, const char *fname, float freq) {
  ...
  for (int p = 0; p < MGetPinCount(); p++) {
    ...
    if (pname[0] == 'P') {
      ...
      avr_irq_t *stateIrq = avr_io_getirq(
          avr, AVR_IOCTL_IOPORT_GETIRQ(*pins[p].port), pins[p].pord);
      avr_irq_register_notify(stateIrq, out_hook, &pins[p]);

      avr_irq_t *directionIrq =
          avr_io_getirq(avr, AVR_IOCTL_IOPORT_GETIRQ(*pins[p].port),
      ...
      avr_irq_t *writeIrq = avr_io_getirq(
          avr, AVR_IOCTL_IOPORT_GETIRQ(*pins[p].port), pins[p].pord);
  ...
  if (has_usart) {
    ...
    serial_irq =
        avr_alloc_irq(&avr->irq_pool, 0, IRQ_UART_COUNT, irq_names_uart);
    ...
    avr_irq_t *dst =
        avr_io_getirq(avr, AVR_IOCTL_UART_GETIRQ('0'), UART_IRQ_INPUT);
    if (src && dst) {
...
float bsim_simavr::MGetFreq(void) { return avr->frequency; }

void bsim_simavr::MSetVCC(float vcc) { avr->vcc = vcc * 1000; }

float bsim_simavr::MGetVCC(void) { return avr->vcc / 1000.0; }

float bsim_simavr::MGetInstClockFreq(void) { return avr->frequency; }

void bsim_simavr::MDumpMemory(const char *fname) { write_ihx_avr(fname); }
...
int bsim_simavr::CpuInitialized(void) { return (avr != NULL); }
...
void bsim_simavr::MSetAPin(int pin, float value) {
  ...
  if (lxString(avr->mmcu).compare(lxT("atmega2560")) == 0) {
    ...
    case 82:
      ...
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 15),
                    (int)(value * 1000));
      break;
    case 83:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 14),
                    (int)(value * 1000));
      break;
    case 84:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 13),
                    (int)(value * 1000));
      break;
    case 85:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 12),
                    (int)(value * 1000));
      break;
    case 86:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 11),
                    (int)(value * 1000));
      break;
    case 87:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 10),
                    (int)(value * 1000));
      break;
    case 88:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 9),
                    (int)(value * 1000));
      break;
    case 89:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 8),
                    (int)(value * 1000));
      break;
    case 90:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 7),
                    (int)(value * 1000));
      break;
    case 91:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 6),
                    (int)(value * 1000));
      break;
    case 92:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 5),
                    (int)(value * 1000));
      break;
    case 93:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 4),
                    (int)(value * 1000));
      break;
    case 94:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 3),
                    (int)(value * 1000));
      break;
    case 95:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 2),
                    (int)(value * 1000));
      break;
    case 96:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 1),
                    (int)(value * 1000));
      break;
    case 97:
      pins[pin - 1].ptype = PT_ANALOG;
      avr_raise_irq(avr_io_getirq(avr, AVR_IOCTL_ADC_GETIRQ, 0),
                    (int)(value * 1000));
      break;
    }
  } else if (lxString(avr->mmcu).compare(lxT("attiny85")) == 0) {
...
const picpin *bsim_simavr::MGetPinsValues(void) { return pins; }
...
void bsim_simavr::UpdateHardware(void) {
    ...
    if (avr->data[UCSR0B] & 0x18) // RXEN TXEN
    {
      if (!uart_config) {
        ...
        if (avr->data[UCSR0A] & 0x02) // U2Xn
        {
          serialexbaud =
              avr->frequency /
              (8 * (((avr->data[UBRR0H] << 8) | avr->data[UBRR0L]) + 1));
        } else {
          serialexbaud =
              avr->frequency /
              (16 * (((avr->data[UBRR0H] << 8) | avr->data[UBRR0L]) + 1));
        }
      ...
      pins[pin_tx - 1].value =
          bitbang_uart_io(&bb_uart, pins[pin_rx - 1].value);
    } else {
      uart_config = 0;
    }
  } else // USI
  {
    ...
    // i2c start
    if ((USI.sdao == 1) && (USI.sda->value == 0) && (USI.scl->value == 1) &&
        (USI.sclo == 1)) // start
    {
    ...
    // i2c stop
    if ((USI.sdao == 0) && (USI.sda->value == 1) && (USI.scl->value == 1) &&
        (USI.sclo == 1)) {
...
void bsim_simavr::MStep(void) { avr_run(avr); }
...
unsigned short *bsim_simavr::DBGGetProcID_p(void) { return 0; }

unsigned int bsim_simavr::DBGGetPC(void) { return avr->pc >> 1; }

void bsim_simavr::DBGSetPC(unsigned int pc) { avr->pc = pc << 1; }

unsigned char *bsim_simavr::DBGGetRAM_p(void) { return avr->data; }

unsigned char *bsim_simavr::DBGGetROM_p(void) { return avr->flash; }

unsigned char *bsim_simavr::DBGGetCONFIG_p(void) { return avr->fuse; }
...
unsigned char *bsim_simavr::DBGGetEEPROM_p(void) { return eeprom; }

unsigned int bsim_simavr::DBGGetRAMSize(void) { return avr->ramend + 1; }

unsigned int bsim_simavr::DBGGetROMSize(void) { return avr->flashend + 1; }
...
unsigned int bsim_simavr::DBGGetEEPROM_Size(void) { return avr->e2end + 1; }

void bsim_simavr::EndServers(void) { mplabxd_server_end(); }
  ...
  if (fin) {
    do {
      ...
      if (checksum(line) == 0) {
        ...
        case 2:
          addrh =
              (((parse_hex(line + 9, 2) << 8) | parse_hex(line + 11, 2))) >> 12;
          break;
        case 4:
