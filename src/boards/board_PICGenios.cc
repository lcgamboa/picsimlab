...
cboard_PICGenios::cboard_PICGenios(void)
  ...
  image.LoadFile(
      lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/VT1.svg")));
  vent[0] = new lxBitmap(&image, &Window1);
  image.LoadFile(
      lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/VT2.svg")));
  vent[1] = new lxBitmap(&image, &Window1);

  image.LoadFile(
      lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/lcd2.svg")));
  lcdbmp[0] = new lxBitmap(&image, &Window1);
  image.LoadFile(
      lxGetLocalFile(Window1.GetSharePath() + lxT("boards/Common/lcd4.svg")));
  ...
  snprintf(mi2c_tmp_name, 200, "%s/picsimlab-XXXXXX",
           (const char *)lxGetTempDir("PICSimLab").c_str());
...
int cboard_PICGenios::MInit(const char *processor, const char *fname,
                            float freq) {
  char fnamem[1024];
  FILE *fout;
...
void cboard_PICGenios::SetScale(double scale) {
  if (Scale != scale) {
    ...
    if (vent[0]) {
      ...
      image.LoadFile(lxGetLocalFile(Window1.GetSharePath() +
                                    lxT("boards/Common/lcd2.svg")),
                     0, Scale, Scale);
      lcdbmp[0] = new lxBitmap(&image, &Window1);
      image.LoadFile(lxGetLocalFile(Window1.GetSharePath() +
                                    lxT("boards/Common/lcd4.svg")),
                     0, Scale, Scale);
      lcdbmp[1] = new lxBitmap(&image, &Window1);
...
void cboard_PICGenios::MDumpMemory(const char *mfname) {
  FILE *fout;
  char fname[1024];
...
void cboard_PICGenios::Draw(CDraw *draw) {
  int i;
  int update = 0;
  ...
  for (i = 0; i < outputc; i++) {
    if (output[i].update) {
      ...
      if (!output[i].r) // rectangle
      {
        ...
        switch (output[i].id) {
        case O_A1:
          draw->Canvas.SetBgColor(0, lm1[18], 0);
          break;
        case O_B1:
          draw->Canvas.SetBgColor(0, lm1[19], 0);
          break;
        case O_C1:
          draw->Canvas.SetBgColor(0, lm1[20], 0);
          break;
        case O_D1:
          draw->Canvas.SetBgColor(0, lm1[21], 0);
          break;
        case O_E1:
          draw->Canvas.SetBgColor(0, lm1[26], 0);
          break;
        case O_F1:
          draw->Canvas.SetBgColor(0, lm1[27], 0);
          break;
        case O_G1:
          draw->Canvas.SetBgColor(0, lm1[28], 0);
          break;

        case O_A2:
          draw->Canvas.SetBgColor(0, lm2[18], 0);
          break;
        case O_B2:
          draw->Canvas.SetBgColor(0, lm2[19], 0);
          break;
        case O_C2:
          draw->Canvas.SetBgColor(0, lm2[20], 0);
          break;
        case O_D2:
          draw->Canvas.SetBgColor(0, lm2[21], 0);
          break;
        case O_E2:
          draw->Canvas.SetBgColor(0, lm2[26], 0);
          break;
        case O_F2:
          draw->Canvas.SetBgColor(0, lm2[27], 0);
          break;
        case O_G2:
          draw->Canvas.SetBgColor(0, lm2[28], 0);
          break;

        case O_A3:
          draw->Canvas.SetBgColor(0, lm3[18], 0);
          break;
        case O_B3:
          draw->Canvas.SetBgColor(0, lm3[19], 0);
          break;
        case O_C3:
          draw->Canvas.SetBgColor(0, lm3[20], 0);
          break;
        case O_D3:
          draw->Canvas.SetBgColor(0, lm3[21], 0);
          break;
        case O_E3:
          draw->Canvas.SetBgColor(0, lm3[26], 0);
          break;
        case O_F3:
          draw->Canvas.SetBgColor(0, lm3[27], 0);
          break;
        case O_G3:
          draw->Canvas.SetBgColor(0, lm3[28], 0);
          break;

        case O_A4:
          draw->Canvas.SetBgColor(0, lm4[18], 0);
          break;
        case O_B4:
          draw->Canvas.SetBgColor(0, lm4[19], 0);
          break;
        case O_C4:
          draw->Canvas.SetBgColor(0, lm4[20], 0);
          break;
        case O_D4:
          draw->Canvas.SetBgColor(0, lm4[21], 0);
          break;
        case O_E4:
          draw->Canvas.SetBgColor(0, lm4[26], 0);
          break;
        case O_F4:
          draw->Canvas.SetBgColor(0, lm4[27], 0);
          break;
        case O_G4:
          draw->Canvas.SetBgColor(0, lm4[28], 0);
          break;
        ...
        if ((output[i].id >= O_BRB0) && (output[i].id <= O_TCT)) {
          ...
          int x =
              -15 * sin((5.585 * (pot[output[i].id - O_POT1] / 200.0)) + 0.349);
          int y =
              15 * cos((5.585 * (pot[output[i].id - O_POT1] / 200.0)) + 0.349);
        ...
        if (output[i].id == O_VT) {
          ...
          draw->Canvas.PutBitmap(vent[vt], output[i].x1 * Scale,
                                 output[i].y1 * Scale);
          ...
          if (lcd.update) {
            draw->Canvas.ChangeScale(1.0, 1.0);
            if (lcd.lnum == 2) {
              draw->Canvas.PutBitmap(lcdbmp[0], (output[i].x1 - 41) * Scale,
                                     (output[i].y1 - 58) * Scale);
            } else {
              draw->Canvas.PutBitmap(lcdbmp[1], (output[i].x1 - 41) * Scale,
                                     (output[i].y1 - 58) * Scale);
            }
            draw->Canvas.ChangeScale(Scale, Scale);
            draw->Canvas.Rectangle(1, output[i].x1 - 1, output[i].y1 - 2,
                                   output[i].x2 - output[i].x1 + 2,
                                   output[i].y2 - output[i].y1 +
                                       ((lcd.lnum == 2) ? 3 : 78));
            if (dip[0]) {
              lcd_draw(&lcd, &draw->Canvas, output[i].x1, output[i].y1,
                       output[i].x2 - output[i].x1, output[i].y2 - output[i].y1,
                       Window1.Get_mcupwr());
            }
          }
        } else if ((output[i].name[0] == 'D') && (output[i].name[1] == 'P')) {
          if (dip[(((output[i].name[3] - 0x30) * 10) +
                   (output[i].name[4] - 0x30)) -
                  1]) {
            draw->Canvas.SetBgColor(70, 70, 70);
            draw->Canvas.Rectangle(
                1, output[i].x1,
                output[i].y1 + ((int)((output[i].y2 - output[i].y1) * 0.35)),
                output[i].x2 - output[i].x1,
          ...
          if (!jmp[0]) {
            ...
            draw->Canvas.Circle(
                1, output[i].x1 + (int)((output[i].x2 - output[i].x1) * 0.80),
                output[i].y1 + ((output[i].y2 - output[i].y1) / 2), 3);
          } else {
            draw->Canvas.SetColor(70, 70, 70);
            draw->Canvas.Rectangle(
                1, output[i].x1 + ((int)((output[i].x2 - output[i].x1) * 0.35)),
                output[i].y1, (int)((output[i].x2 - output[i].x1) * 0.65),
                output[i].y2 - output[i].y1);
            draw->Canvas.SetColor(220, 220, 0);
            draw->Canvas.Circle(
                1, output[i].x1 + (int)((output[i].x2 - output[i].x1) * 0.20),
                output[i].y1 + ((output[i].y2 - output[i].y1) / 2), 3);
          }
          ...
          draw->Canvas.RotatedText(Proc, output[i].x1 + 20, output[i].y1 + 50,
                                   -90);
        }
      ...
      } else // circle output
      {
        ...
        if (dip[9])
          ...
          case O_RB3:
            ...
            break;
          case O_RB4:
            draw->Canvas.SetBgColor(pic.pins[36].oavalue, 0, 0);
            break;
          case O_RB5:
            draw->Canvas.SetBgColor(pic.pins[37].oavalue, 0, 0);
            break;
          case O_RB6:
            draw->Canvas.SetBgColor(pic.pins[38].oavalue, 0, 0);
            break;
          case O_RB7:
            draw->Canvas.SetBgColor(pic.pins[39].oavalue, 0, 0);
            break;
          }
        ...
        if (dip[8])
          switch (output[i].id) {
          case O_RD0:
            draw->Canvas.SetBgColor(pic.pins[18].oavalue, 0, 0);
            break;
          case O_RD1:
            draw->Canvas.SetBgColor(pic.pins[19].oavalue, 0, 0);
            break;
          case O_RD2:
            draw->Canvas.SetBgColor(pic.pins[20].oavalue, 0, 0);
            break;
          case O_RD3:
            draw->Canvas.SetBgColor(pic.pins[21].oavalue, 0, 0);
            break;
          case O_RD4:
            draw->Canvas.SetBgColor(pic.pins[26].oavalue, 0, 0);
            break;
          case O_RD5:
            draw->Canvas.SetBgColor(pic.pins[27].oavalue, 0, 0);
            break;
          case O_RD6:
            draw->Canvas.SetBgColor(pic.pins[28].oavalue, 0, 0);
            break;
          case O_RD7:
            draw->Canvas.SetBgColor(pic.pins[29].oavalue, 0, 0);
            break;
          }
  ...
  // buzzer
  if ((((pic.pins[15].oavalue - 55) / 2) > 10) && (Window1.Get_mcupwr()) &&
      jmp[0]) {
    if (!sound_on) {
      buzzer.BeepStart();
      sound_on = 1;
  ...
  // thacometer
  rpmstp =
      ((float)Window1.GetNSTEPJ()) / (0.7196 * (pic.pins[16].oavalue - 54));
  ...
  // temp
  ref = ((0.25 * (pic.pins[23].oavalue - 55))) -
        (0.25 * (pic.pins[16].oavalue - 55));
...
void cboard_PICGenios::Run_CPU(void) {
  ...
  for (int pl = 0; pl < 6; pl++) {
    if ((pins[33 + pl - 1].dir == PD_IN) &&
        (pins[33 + pl - 1].value != p_BT_[pl])) {
  ...
  if (Window1.Get_mcupwr())
    for (i = 0; i < NSTEP; i++) {
      ...
      if (ioupdated) {
        ...
        pic_set_pin(&pic, 23,
                    mi2c_io(&mi2c, sck, sda) &
                        rtc_ds1307_I2C_io(&rtc2, sck, sda));
      }
      pic.ioupdated = 0;
    }
...
void cboard_PICGenios::EvMouseButtonPress(uint button, uint x, uint y,
                                          uint state) {
  int i;
...
void cboard_PICGenios::EvMouseButtonRelease(uint button, uint x, uint y,
                                            uint state) {
  int i;
...
void cboard_PICGenios::RefreshStatus(void) {
  label5->SetText(lxT("Temp: ") + lxString().Format("%5.2f", temp[0]) +
                  lxT("°C"));
...
void cboard_PICGenios::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("PICGenios_clock"),
