...
void cboard_McLab2::Draw(CDraw *draw) {
    ...
    if (output[i].update) // only if need update
    {
      ...
      if (!output[i].r) // rectangle
      {
        ...
        if ((output[i].id >= O_BRB0) && (output[i].id <= O_BRB3)) {
          ...
          draw->Canvas.RotatedText(Proc, output[i].x1 + 10, output[i].y2 - 50,
                                   90);
  ...
  // thacometer
  rpmstp =
      ((float)Window1.GetNSTEPJ()) / (0.7196 * (pic.pins[15].oavalue - 54));

  // potentiometer
  vp2in = (5.0 * pot1 / 199);

  // temp
  ref = (0.25 * (pic.pins[16].oavalue - 55)) -
        (0.25 * (pic.pins[15].oavalue - 55));
...
void cboard_McLab2::EvMouseButtonPress(uint button, uint x, uint y,
                                       uint state) {
  int i;
...
void cboard_McLab2::EvMouseButtonRelease(uint button, uint x, uint y,
                                         uint state) {
  int i;
...
void cboard_McLab2::RefreshStatus(void) {
  label4->SetText(lxT("Temp: ") + lxString().Format("%5.2f", temp[0]) +
                  lxT("°C"));
...
void cboard_McLab2::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("McLab2_clock"),
