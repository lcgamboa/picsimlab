    ...
    if (output[i].update) // only if need update
    {
      ...
      if (!output[i].r) // if output shape is a rectangle
      {
        if (output[i].id == O_SD1) // if output is switch
        {
          ...
          draw->Canvas.Rectangle(1, output[i].x1 + pot1 / 2.77,
                                 output[i].y1 + 2, 10, 15);
      ...
      } else // if output shape is a circle
      {
        ...
        case O_LD0: // White using pin 19 mean value (RD0)
          draw->Canvas.SetBgColor(pic.pins[18].oavalue, pic.pins[18].oavalue,
                                  pic.pins[18].oavalue);
          break;
        case O_LD1: // Yelllow using pin 20 mean value (RD1)
          draw->Canvas.SetBgColor(pic.pins[19].oavalue, pic.pins[19].oavalue,
                                  0);
          break;
