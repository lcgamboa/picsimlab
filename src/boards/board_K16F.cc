    ...
    if (output[i].update) // only if need update
    {
      ...
      if (!output[i].r) // rectangle
      {
        ...
        if (output[i].id == O_LCD) {
          ...
          draw->Canvas.Rectangle(0, output[i].x1 + 3, output[i].y1 + 3,
                                 output[i].x2 - output[i].x1 - 6,
      ...
      if (ioupdated) {
        ...
        pic_set_pin(&pic, 3,
                    mi2c_io(&mi2c, sck, sda) &
                        rtc_pfc8563_I2C_io(&rtc, sck, sda));
      }
      pic.ioupdated = 0;
    }
  // fim STEP
...
void cboard_K16F::EvMouseButtonRelease(uint button, uint x, uint y,
                                       uint state) {
  int i;
...
void cboard_K16F::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("K16F_clock"),
