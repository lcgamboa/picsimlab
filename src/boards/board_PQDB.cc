...
void cboard_PQDB::Draw(CDraw * draw, double scale) {
    int i;
    ...
    if (((0.4444 * (pic.pins[PWM_PIN].oavalue - 30)) > 10) && (Window1.Get_mcupwr())) {
        if (!sound_on) {
            buzzer.BeepStart();
            sound_on = 1;
        }
