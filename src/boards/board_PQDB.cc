...
void cboard_PQDB::Draw(CDraw *draw) {
    ...
    if (output[i].update) // only if need update
    {
      ...
      if (!output[i].r) {
        ...
        case O_RST:
          ...
          draw->Canvas.Rectangle(1, output[i].x1 + 1, output[i].y1 + 1,
                                 output[i].x2 - output[i].x1 - 1,
        ...
        if (output[i].id == O_LCD) {
          ...
          draw->Canvas.Rectangle(1, output[i].x1 - 8, output[i].y1 - 10,
                                 output[i].x2 - output[i].x1 + 17,
        ...
        if (output[i].id == O_RGB) {
          draw->Canvas.SetColor(pic.pins[LED_RED_PIN].oavalue,
                                pic.pins[LED_GREEN_PIN].oavalue,
  ...
  if (((0.5 * (pic.pins[PWM_PIN].oavalue - 55)) > 10) &&
      (Window1.Get_mcupwr())) {
    if (!sound_on) {
      buzzer.BeepStart();
      sound_on = 1;
...
void cboard_PQDB::Run_CPU(void) {
  ...
  // sr mean value
  pic.pins[PSRD0].oavalue =
      (pic.pins[PSRD0].oavalue + ((shiftReg_alm[0] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD1].oavalue =
      (pic.pins[PSRD1].oavalue + ((shiftReg_alm[1] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD2].oavalue =
      (pic.pins[PSRD2].oavalue + ((shiftReg_alm[2] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD3].oavalue =
      (pic.pins[PSRD3].oavalue + ((shiftReg_alm[3] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD4].oavalue =
      (pic.pins[PSRD4].oavalue + ((shiftReg_alm[4] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD5].oavalue =
      (pic.pins[PSRD5].oavalue + ((shiftReg_alm[5] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD6].oavalue =
      (pic.pins[PSRD6].oavalue + ((shiftReg_alm[6] * 200.0) / NSTEPJ) + 55) / 2;
  pic.pins[PSRD7].oavalue =
      (pic.pins[PSRD7].oavalue + ((shiftReg_alm[7] * 200.0) / NSTEPJ) + 55) / 2;
  if (use_spare)
    Window5.PostProcess();
...
void cboard_PQDB::EvMouseButtonRelease(uint button, uint x, uint y,
                                       uint state) {
  int i;
...
unsigned short cboard_PQDB::get_in_id(char *name) {
  ...
  if (strcmp(name, "PB_SW1") == 0)
    return I_TC1;
  if (strcmp(name, "PB_SW2") == 0)
    return I_TC2;
  if (strcmp(name, "PB_SW3") == 0)
    return I_TC3;
  if (strcmp(name, "PB_SW4") == 0)
    return I_TC4;
  if (strcmp(name, "PB_SW5") == 0)
    return I_TC5;
  if (strcmp(name, "PB_SW6") == 0)
    return I_TC6;
  if (strcmp(name, "PB_SW7") == 0)
    return I_TC7;
  if (strcmp(name, "PB_SW8") == 0)
    return I_TC8;
  if (strcmp(name, "PB_SW9") == 0)
    return I_TC9;
  if (strcmp(name, "PB_SW10") == 0)
    return I_TC0;
...
void cboard_PQDB::WritePreferences(void) {
  ...
  Window1.saveprefs(lxT("PQDB_clock"),
...
int cboard_PQDB::MGetPinCount(void) { return bsim_picsim::MGetPinCount() + 8; }
