...
void cpart_MI2C_24CXXX::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        case O_IC:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{buff, output[i].x1, output[i].y2 - 15, 0}});
            break;
        default:
...
void cpart_MI2C_24CXXX::ReadPreferences(std::string value) {
    sscanf(value.c_str(), "%hhu,%hhu,%hhu,%hhu,%hhu,%u,%[^\n]", &input_pins[0], &input_pins[1], &input_pins[2],
