    ...
    switch (output[i].id) {
        case O_P1:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{temp.c_str(), output[i].x1, output[i].y1, 0}});
            if (output_pins[0] == 0)
                SpareParts.CanvasCmd(
                    {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"NC", output[i].x1, output[i].y1 + 12, 0}});
            else
                SpareParts.CanvasCmd(
                    {.partn = id,
                     .cmd = CC_ROTATEDTEXT,
                     .RotatedText{SpareParts.GetPinName(output_pins[0]).c_str(), output[i].x1, output[i].y1 + 12, 0}});
            break;
        case O_P2:
        case O_P3:
        case O_P4:
        ...
        case O_B7:
        case O_B8:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{100, 100, 100}});
