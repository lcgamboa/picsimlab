...
void cpart_SignalGenerator::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_P3:
            ...
            if (input_pins[output[i].id - O_P2] == 0)
                ...
                SpareParts.CanvasCmd({.partn = id,
                                      .cmd = CC_ROTATEDTEXT,
                                      .RotatedText{SpareParts.GetPinName(input_pins[output[i].id - O_P2]).c_str(),
        ...
        case O_PO2:
        case O_PO3:
            DrawPotentiometer(id, SpareParts.CanvasCmd, &output[i], 200 - values[output[i].id - O_PO1], "", 9);
            break;
        case O_TP:
            ...
            for (j = 1; j < sizex; j++) {
                ...
                if (j > 0) {
                    SpareParts.CanvasCmd({.partn = id,
                                          .cmd = CC_LINE,
                                          .Line{output[i].x1 + j - 1, output[i].y1 + ((v[1] + 2.0f) * sizey / 4.0f),
        ...
        case O_AMPL:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{255, 255, 255}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{temp, output[i].x1, output[i].y1, 0}});
            break;
        case O_OFFS:
        ...
        case O_MF:
            ...
            SpareParts.CanvasCmd(
                {.partn = id,
                 .cmd = CC_RECTANGLE,
                 .Rectangle{1, output[i].x1, output[i].y1, output[i].x2 - output[i].x1, output[i].y2 - output[i].y1}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, 0, 0}});
