...
void cpart_IO_MCP23S17::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        case O_IC:
            ...
            SpareParts.CanvasCmd({CC_ROTATEDTEXT, .RotatedText{"MCP23S17", output[i].x1 + 50, output[i].y2 - 15, 0.0}});
            break;
        default:
            ...
            if (pinv > 25) {
                ...
                if (output_pins[pinv - 10] == 0)
                    ...
                    SpareParts.CanvasCmd({CC_ROTATEDTEXT, .RotatedText{std::to_string(output_pins[pinv - 10]).c_str(),
                                                                       output[i].x1, output[i].y2 - 30, 90.0}});
            }
            break;
    }
