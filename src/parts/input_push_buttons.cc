...
void cpart_pbuttons::ConfigurePropertiesWindow(void) {
    ...
    SpareParts.WPropCmd("combo11", PWA_COMBOSETITEMS, "HIGH,LOW,");
    if (active)
        SpareParts.WPropCmd("combo11", PWA_COMBOSETTEXT, "HIGH");
    else
        SpareParts.WPropCmd("combo11", PWA_COMBOSETTEXT, "LOW ");
...
void cpart_pbuttons::SpinChange(const char* controlname, int value) {
    for (int i = 0; i < 8; i++) {
        char name[20];
