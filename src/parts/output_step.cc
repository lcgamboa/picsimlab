...
void cpart_step::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_STEPS: {
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{text, output[i].x1, output[i].y1, 0}});
            snprintf(text, 100, "T=%3i", turns);
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{text, output[i].x1, output[i].y1 + 12, 0}});
        } break;
        case O_ROT:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{77, 77, 77}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].x1, output[i].y1, output[i].r + 10}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{0, 0, 0}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{250, 250, 250}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].x1, output[i].y1, output[i].r}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{55, 55, 55}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].x1, output[i].y1, output[i].r / 3}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{8}});
            x2 = output[i].x1 + (output[i].r - 2) * sin(angle);
            y2 = output[i].y1 + (output[i].r - 2) * cos(angle);
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_LINE, .Line{output[i].x1, output[i].y1, x2, y2}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{6}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{77, 77, 77}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_LINE, .Line{output[i].x1, output[i].y1, x2, y2}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{1}});
            break;
        case O_L1:
        case O_L2:
        case O_L3:
