...
void cpart_rgb_led::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_L1:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{255, 255, 224}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].x1, output[i].y1, output[i].r - 5}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{0, 0, 0}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{color[0], color[1], color[2]}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].x1 - 0.5f, output[i].y1, output[i].r - 7.0f}});
            break;
    }
}
