...
void cpart_encoder::PreProcess(void) {
    ...
    if (output_pins[2])
        Window5.SetPin(output_pins[2], p_BTN);
}
...
void cpart_encoder::EvMouseButtonPress(uint button, uint x, uint y, uint state) {
    int i;
...
void cpart_encoder::EvMouseButtonRelease(uint button, uint x, uint y, uint state) {
    int i;
...
void cpart_encoder::EvMouseMove(uint button, uint x, uint y, uint state) {
    int i;
...
unsigned char cpart_encoder::CalcAngle(int i, int x, int y) {
    int dx = input[i].cx - x;
    int dy = y - input[i].cy;
    double angle = 0;
...
unsigned short cpart_encoder::get_in_id(char* name) {
...
unsigned short cpart_encoder::get_out_id(char* name) {
