...
void cpart_7s_display::Draw(void) {
    int i;
            ...
            switch (output[i].id) {
                ...
                case O_DISP4:
                    ...
                    if (input_pins[output[i].id - O_SA] == 0)
                        ...
                        canvas.RotatedText(SpareParts.GetPinName(input_pins[output[i].id - O_SA]), output[i].x1,
                                           output[i].y2, 90.0);
                    break;
                case O_FX1:
                    canvas.SetColor(49, 61, 99);
...
unsigned short cpart_7s_display::get_in_id(char* name) {
    printf("Erro input '%s' don't have a valid id! \n", name);
    return -1;
}

unsigned short cpart_7s_display::get_out_id(char* name) {
...
void cpart_7s_display::ConfigurePropertiesWindow(CPWindow* WProp) {
    ...
    if (input_pins[11] == 0)
        ...
        spin = SpareParts.GetPinName(input_pins[11]);
