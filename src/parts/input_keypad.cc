...
void cpart_keypad::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        ...
        case O_K1 ... O_KD:
            if (output[i].value) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{4}});
                ...
                if ((output[i].id == O_Ka) || (output[i].id == O_KT) || !((output[i].id + 1) % 4)) {
                    SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{190, 46, 37}});
                } else {
                    SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{50, 118, 179}});
                }
            }
