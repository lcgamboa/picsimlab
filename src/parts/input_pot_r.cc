...
void cpart_pot_r::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_PO4:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{66, 109, 246}});
            SpareParts.CanvasCmd(
                {.partn = id,
                 .cmd = CC_RECTANGLE,
                 .Rectangle{1, output[i].x1, output[i].y1, output[i].x2 - output[i].x1, output[i].y2 - output[i].y1}});
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 17}});

            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{150, 150, 150}});
            int x = -13 * sin((5.585 * (values[output[i].id - O_PO1] / 200.0)) + 0.349);
            int y = 13 * cos((5.585 * (values[output[i].id - O_PO1] / 200.0)) + 0.349);
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx + x, output[i].cy + y, 2}});

            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_RECTANGLE, .Rectangle{1, output[i].x1 + 6, output[i].y2 + 6, 30, 15}});
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{val, output[i].x1 + 6, output[i].y2 + 6, 0}});
            break;
    }
}
...
int cpart_pot_r::ReadPreferences(std::string value) {
    unsigned int sz;
