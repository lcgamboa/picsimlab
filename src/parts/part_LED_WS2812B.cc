...
void cpart_led_ws2812b::LoadImage(void) {
  ...
  if (led.nleds > 1) {
    ...
    image.LoadFile(lxGetLocalFile(Window1.GetSharePath() + lxT("parts/") +
                                  GetPictureFileName()),
    ...
    if (!led.diffuser) {
      for (unsigned int r = 0; r < led.nrows; r++)
        for (unsigned int c = 0; c < led.ncols; c++) {
          canvas.PutBitmap(LEDBitmap,
                           (output_ids[O_LED]->x1 + (40 * c)) * Scale,
  ...
  for (i = 0; i < outputc; i++) {
    if (output[i].update) // only if need update
    {
      ...
      case O_P1:
        ...
        canvas.Rectangle(1, output[i].x1, output[i].y1 + yoff,
        ...
        if (input_pins[output[i].id - O_P1] == 0)
          ...
          canvas.RotatedText(
              Window5.GetPinName(input_pins[output[i].id - O_P1]), output[i].x1,
      ...
      case O_P2:
        ...
        if (output_pins[output[i].id - O_P2] == 0)
          ...
          canvas.RotatedText(itoa(output_pins[output[i].id - O_P2]),
      ...
      case O_LED:
        ...
        if (led.update) {
          ...
          led_ws2812b_draw(&led, &canvas, output[i].x1, output[i].y1 + yoff,
                           output[i].x2 - output[i].x1,
...
lxString cpart_led_ws2812b::WritePreferences(void) {
  ...
  sprintf(prefs, "%hhu,%hhu,%u,%u,%hhu", input_pins[0], output_pins[0],
          led.nrows, led.ncols, led.diffuser);
...
void cpart_led_ws2812b::ReadPreferences(lxString value) {
  ...
  sscanf(value.c_str(), "%hhu,%hhu,%u,%u,%hhu", &input_pins[0], &outp, &rows,
         &cols, &diffuser);
...
void cpart_led_ws2812b::ChangeType(const unsigned int rows,
                                   const unsigned int cols,
                                   const unsigned char diffuser) {
  if ((led.nrows != rows) || (led.ncols != cols) ||
...
void cpart_led_ws2812b::ConfigurePropertiesWindow(CPWindow *WProp) {
  ...
  if (input_pins[0] == 0)
    ...
    ((CCombo *)WProp->GetChildByName("combo1"))
        ->SetText(itoa(input_pins[0]) + "  " + spin);
  }

  ((CLabel *)WProp->GetChildByName("label3"))
      ->SetText("Pin 2 - DOUT     " + itoa(output_pins[0]));
