...
void cpart_UART::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        case O_LTX:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, (sr.bb_uart.leds & 0x02) * 125, 0}});
            ...
            break;
        case O_LRX:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, (sr.bb_uart.leds & 0x01) * 250, 0}});
            SpareParts.CanvasCmd(
                {.partn = id,
                 .cmd = CC_RECTANGLE,
                 .Rectangle{1, output[i].x1, output[i].y1, output[i].x2 - output[i].x1, output[i].y2 - output[i].y1}});
