...
void cpart_LCD_hd44780::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_P11:
            ...
            if (input_pins[output[i].id - O_P1] == 0)
                SpareParts.CanvasCmd(
                    {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"NC", output[i].x1, output[i].y2 + yoff, 90.0}});
            else
                SpareParts.CanvasCmd({.partn = id,
                                      .cmd = CC_ROTATEDTEXT,
        ...
        case O_F1:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"GND", output[i].x1, output[i].y2 + yoff, 90.0}});
            break;
        case O_F2:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd({.partn = id,
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{155, 155, 155}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"+5V", output[i].x1, output[i].y2 + yoff, 90.0}});
            break;
        case O_F3:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd({.partn = id,
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{155, 155, 155}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"POT", output[i].x1, output[i].y2 + yoff, 90.0}});
            break;
        case O_LCD:
            // draw lcd text
            if (lcd.update) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, 90 + 40, 0}});
                lcd_draw(&lcd, id, SpareParts.CanvasCmd, output[i].x1, output[i].y1, output[i].x2 - output[i].x1,
                         output[i].y2 - output[i].y1, 1);
            }
            /*
            else
            {
               SpareParts.CanvasCmd( {.partn=id, .cmd = CC_RECTANGLE, .Rectangle{11, output[i].x1, output[i].y1,
...
int cpart_LCD_hd44780::ReadPreferences(std::string value) {
    int ret = sscanf(value.c_str(), "%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu,%hhu", &input_pins[0],
                     &input_pins[1], &input_pins[2], &input_pins[3], &input_pins[4], &input_pins[5], &input_pins[6],
                     &input_pins[7], &input_pins[8], &input_pins[9], &input_pins[10], &model);
    Reset();
    return ret;
}
...
static part* cpart_LCD_hd44780_i2c_create(const unsigned int x, const unsigned int y, board* pboard_, const int id_) {
    cpart_IO_PCF8574* pcf = (cpart_IO_PCF8574*)SpareParts.AddPart("IO PCF8574", x, y + 365);
