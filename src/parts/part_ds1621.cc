...
unsigned short cpart_ds1621::get_in_id(char* name) {
...
unsigned short cpart_ds1621::get_out_id(char* name) {
...
void cpart_ds1621::Process(void) {
    ...
    if ((input_pins[0] > 0) && (input_pins[1] > 0))
        Window5.Set_i2c_bus(input_pins[0] - 1,
                            sen_ds1621_I2C_io(&ds1621, ppins[input_pins[1] - 1].value, ppins[input_pins[0] - 1].value));
