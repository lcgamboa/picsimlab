...
void cpart_switches::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        ...
        case O_S8:
            ...
            if (active) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{100, 60, 60}});
            } else {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{60, 100, 60}});
            }
            SpareParts.CanvasCmd(
                {.partn = id,
                 .cmd = CC_RECTANGLE,
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{0, 0, 0}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{26, 26, 26}});
            SpareParts.CanvasCmd(
                {.partn = id,
                 .cmd = CC_RECTANGLE,
