...
cpart_dht11::cpart_dht11(unsigned x, unsigned y)
    ...
    sen_dhtxx_init(&dht11, Window1.GetBoard(), DHT11);
}
...
void cpart_dht11::Process(void) {
    if (output_pins[0]) {
        ...
        if ((ppins[output_pins[0] - 1].dir) && (ppins[output_pins[0] - 1].value != dht11.out)) {
            Window5.SetPin(output_pins[0], dht11.out);
        }

        Window5.Set_pullup_bus(output_pins[0] - 1, sen_dhtxx_io(&dht11, ppins[output_pins[0] - 1].value));
    }
}

void cpart_dht11::EvMouseButtonPress(uint button, uint x, uint y, uint state) {
    int i;
...
void cpart_dht11::EvMouseButtonRelease(uint button, uint x, uint y, uint state) {
    int i;
...
void cpart_dht11::EvMouseMove(uint button, uint x, uint y, uint state) {
    int i;
...
unsigned short cpart_dht11::get_in_id(char* name) {
...
unsigned short cpart_dht11::get_out_id(char* name) {
