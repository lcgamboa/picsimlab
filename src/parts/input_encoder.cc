...
void cpart_encoder::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_RT1:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETBGCOLOR, .SetBgColor{44, 44, 44}});
        ...
        case O_BTN:
            if (p_BTN) {
                ...
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{22, 22, 22}});
            }
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 9}});
            break;
    }
}
