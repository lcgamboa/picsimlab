/* ########################################################################
   ...
   Copyright (c) : 2022-2026  Luis Claudio Gambôa Lopes <lcgamboa@yahoo.com>
...
void cpart_led_ws2812b::LoadPartImage(void) {
    if (led.nleds > 1) {
        ...
        SpareParts.CanvasCmd({.partn = id, .cmd = CC_PUTBITMAP, .PutBitmap{BackBitmap, 0, yoff * Scale}});
            ...
            for (unsigned int r = 0; r < led.nrows; r++)
                for (unsigned int c = 0; c < led.ncols; c++) {
                    SpareParts.CanvasCmd({.partn = id,
                                          .cmd = CC_PUTBITMAP,
                                          .PutBitmap{LEDBitmap, (output_ids[O_LED]->x1 + (40 * c)) * Scale,
        ...
        SpareParts.CanvasCmd({.partn = id, .cmd = CC_FREEBITMAP, .FreeBitmap{LEDBitmap}});
    ...
    switch (output[i].id) {
        ...
        case O_P2:
            ...
            if (output_pins[output[i].id - O_P2] == 0)
                ...
                SpareParts.CanvasCmd({.partn = id,
                                      .cmd = CC_ROTATEDTEXT,
                                      .RotatedText{std::to_string(output_pins[output[i].id - O_P2]).c_str(),
                                                   output[i].x1, output[i].y2 + yoff, 90.0}});
            break;
            break;
        case O_F1:
        ...
        case O_LED:
            ...
            if (led.update) {
                ...
                led_ws2812b_draw(&led, id, SpareParts.CanvasCmd, output[i].x1, output[i].y1 + yoff,
