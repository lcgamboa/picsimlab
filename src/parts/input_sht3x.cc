    ...
    switch (output[i].id) {
        case O_P1:
            ...
            if (output_pins[output[i].id - O_P1] == 0)
                SpareParts.CanvasCmd({.cmd = CC_ROTATEDTEXT, .RotatedText{"2-T   NC", output[i].x1, output[i].y2, 90}});
            else
                SpareParts.CanvasCmd(
                    {.cmd = CC_ROTATEDTEXT,
        ...
        case O_P2:
            ...
            SpareParts.CanvasCmd({.cmd = CC_SETFGCOLOR, .SetFgColor{255, 255, 255}});
            if (output_pins[output[i].id - O_P1] == 0)
                SpareParts.CanvasCmd({.cmd = CC_ROTATEDTEXT, .RotatedText{"3-RH  NC", output[i].x1, output[i].y2, 90}});
            else
                SpareParts.CanvasCmd(
                    {.cmd = CC_ROTATEDTEXT,
