...
unsigned short cpart_RTC_ds1307::get_in_id(char* name) {
    printf("Erro input '%s' don't have a valid id! \n", name);
    return -1;
};

unsigned short cpart_RTC_ds1307::get_out_id(char* name) {
...
void cpart_RTC_ds1307::ReadPreferences(lxString value) {
...
void cpart_RTC_ds1307::Process(void) {
    ...
    if ((input_pins[0] > 0) && (input_pins[1] > 0))
        Window5.Set_i2c_bus(input_pins[0] - 1,
                            rtc_ds1307_I2C_io(&rtc2, ppins[input_pins[1] - 1].value, ppins[input_pins[0] - 1].value));
}
