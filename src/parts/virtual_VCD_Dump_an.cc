...
void cpart_VCD_Dump_an::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_NAME:
            ...
            SpareParts.CanvasCmd({CC_ROTATEDTEXT, .RotatedText{((f_vcd_name + to) + std::string(" (An)")).c_str(),
                                                               output[i].x1, output[i].y1, 0}});
            break;
        case O_L1:
        case O_L2:
...
void cpart_VCD_Dump_an::OnMouseButtonPress(uint inputId, uint button, uint x, uint y, uint state) {
    switch (inputId) {
        case I_START:
            rec ^= 1;
