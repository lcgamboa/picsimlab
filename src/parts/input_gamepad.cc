...
void cpart_gamepad::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        ...
        case O_P8:
            ...
            break;
        case O_J1:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{26, 26, 26}});
            SpareParts.CanvasCmd({.partn = id,
                                  .cmd = CC_CIRCLE,
                                  .Circle{1, output[i].x1 + jr + 10, output[i].y1 + jr + 10, (float)(jr + 10)}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{51, 51, 51}});
            SpareParts.CanvasCmd({.partn = id,
                                  .cmd = CC_CIRCLE,
                                  .Circle{1, output[i].x1 + jr + 10, output[i].y1 + jr + 10, (float)(jr - 4)}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{250, 250, 250}});
            SpareParts.CanvasCmd({.partn = id,
                                  .cmd = CC_CIRCLE,
                                  .Circle{1, output[i].x1 + value[0] + 10, output[i].y1 + value[1] + 10, 8}});
            break;
        case O_B5:
        case O_B6:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{102, 102, 102}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 10}});
            if (output_value[output[i].id - O_B1]) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{15, 15, 15}});
            } else {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{55, 55, 55}});
            }
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 8}});
            break;
        case O_B1:
        case O_B3:
            if (output_value[output[i].id - O_B1]) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{244, 244, 0}});
            } else {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0x9c, 0x94, 0x47}});
            }
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 20}});
            break;
        case O_B2:
        case O_B4:
            if (output_value[output[i].id - O_B1]) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, 0, 214}});
            } else {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{64, 87, 106}});
            }
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_CIRCLE, .Circle{1, output[i].cx, output[i].cy, 20}});
            break;
    }
