    ...
    switch (output[i].id) {
        case O_P1:
            ...
            if (output_pins[output[i].id - O_P1] == 0)
                SpareParts.CanvasCmd(
                    {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"2-D  NC", output[i].x1, output[i].y2, 90}});
            else
                SpareParts.CanvasCmd(
                    {.partn = id,
                     .cmd = CC_ROTATEDTEXT,
        ...
        case O_F1:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"1-VCC +5V", output[i].x1, output[i].y2, 90}});
            break;
        case O_F2:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd(
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{155, 155, 155}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"4-GND GND", output[i].x1, output[i].y2, 90}});
            break;
        case O_F3:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd(
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{155, 155, 155}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"3-NC ", output[i].x1, output[i].y2, 90}});
            break;
        case O_PO1:
            snprintf(val, 10, "%5.1f", (0.6 * (200 - values[0]) - 40));
            DrawSlider(id, SpareParts.CanvasCmd, &output[i], values[0], val, 7);
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFONTSIZE, .SetFontSize{9}});
            break;
        case O_PO2:
            snprintf(val, 10, " %3.0f%%", ((200 - values[1]) / 2.0));
            DrawSlider(id, SpareParts.CanvasCmd, &output[i], values[1], val, 7);
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFONTSIZE, .SetFontSize{9}});
            break;
    }
}
...
int cpart_dht22::ReadPreferences(std::string value_) {
