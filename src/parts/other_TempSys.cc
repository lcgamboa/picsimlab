...
cpart_tempsys::~cpart_tempsys(void) {
    ...
    SpareParts.CanvasCmd({.partn = id, .cmd = CC_FREEBITMAP, .FreeBitmap{vent[0]}});
    SpareParts.CanvasCmd({.partn = id, .cmd = CC_FREEBITMAP, .FreeBitmap{vent[1]}});
    vent[0] = -1;
    vent[1] = -1;
...
void cpart_tempsys::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        ...
        case O_F1:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"12V", output[i].x1, output[i].y1, 0}});
            break;
        case O_F2:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{49, 61, 99}});
            SpareParts.CanvasCmd(
        ...
        case O_OTA:
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{str, output[i].x1, output[i].y1, 0}});
            break;
        case O_OTE:
            ...
            break;
        case O_VT:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_PUTBITMAP, .PutBitmap{vent[vt], output[i].x1, output[i].y1}});
            break;
    }
}
