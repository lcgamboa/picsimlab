...
unsigned short cpart_LCD_ssd1306::get_in_id(char* name) {
    printf("Erro input '%s' don't have a valid id! \n", name);
    return -1;
};

unsigned short cpart_LCD_ssd1306::get_out_id(char* name) {
...
void cpart_LCD_ssd1306::Process(void) {
    ...
    if (type_com) {
        if ((input_pins[0] > 0) && (input_pins[1] > 0))
            Window5.Set_i2c_bus(input_pins[1] - 1, lcd_ssd1306_I2C_io(&lcd, ppins[input_pins[1] - 1].value,
