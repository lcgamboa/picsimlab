...
void cpart_ds1621::DrawOutput(const unsigned int i) {
    ...
    switch (output[i].id) {
        case O_IC:
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFONTSIZE, .SetFontSize{6}});
            ...
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"DS1621", output[i].x1, output[i].y2 - 15, 0.0}});
            break;
        case O_PO1:
            snprintf(val, 10, "%5.1f", (0.9 * (200 - value) - 55));
            DrawSlider(id, SpareParts.CanvasCmd, &output[i], value, val, 6);
