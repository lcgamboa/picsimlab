...
void cpart_LCD_pcf8833::DrawOutput(const unsigned int i) {
    switch (output[i].id) {
        ...
        case O_F5:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{155, 155, 155}});
            SpareParts.CanvasCmd(
                {.partn = id, .cmd = CC_ROTATEDTEXT, .RotatedText{"6V", output[i].x1, output[i].y2, 90.0}});
            break;
        case O_LCD:
            // draw lcd text
            if (lcd.update) {
                SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETCOLOR, .SetColor{0, 90 + 40, 0}});
                lcd_pcf8833_draw(&lcd, id, SpareParts.CanvasCmd, output[i].x1, output[i].y1,
...
int cpart_LCD_pcf8833::ReadPreferences(std::string value) {
    int ret =
