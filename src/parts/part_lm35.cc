...
cpart_lm35::cpart_lm35(unsigned x, unsigned y) :
...
void
cpart_lm35::RegisterRemoteControl(void)
{
   ...
   if (output[i].update)//only if need update
    {
     ...
     switch (output[i].id)
      ...
      case O_PO1:
       ...
       canvas.RotatedText (val, output[i].x1 + 1, output[i].y1 + 5 + value / 1.66, 0);
       canvas.SetFont (font);
       break;
      }
    }
...
void
cpart_lm35::EvMouseButtonPress(uint button, uint x, uint y, uint state)
{
...
void
cpart_lm35::EvMouseButtonRelease(uint button, uint x, uint y, uint state)
{
 int i;
...
void
cpart_lm35::EvMouseMove(uint button, uint x, uint y, uint state)
{
...
unsigned short
cpart_lm35::get_in_id(char * name)
{
...
unsigned short
cpart_lm35::get_out_id(char * name)
{
...
lxString
cpart_lm35::WritePreferences(void)
{
 char prefs[256];
...
void
cpart_lm35::ReadPreferences(lxString value_)
{
 sscanf (value_.c_str (), "%hhu,%hhu", &output_pins[0], &value);
}

void
cpart_lm35::ConfigurePropertiesWindow(CPWindow * WProp)
{
...
void
cpart_lm35::ReadPropertiesWindow(CPWindow * WProp)
{
