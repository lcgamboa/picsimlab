...
const unsigned char* cpart_IO_PCF8574::get_output_pins(void) {
    return output_pins;
}

void cpart_IO_PCF8574::Draw(void) {
    int i;
                ...
                default:
                    ...
                    if (pinv > 13) {
                        ...
                        if (output_pins[pinv - 5] == 0)
                            ...
                            canvas.RotatedText(
                                itoa(
                                    output_pins[pinv - 5]) /*+ lxT (" ") + Window5.GetPinName (output_pins[pinv - 5])*/,
...
unsigned short cpart_IO_PCF8574::get_in_id(char* name) {
    printf("Erro input '%s' don't have a valid id! \n", name);
    return -1;
};

unsigned short cpart_IO_PCF8574::get_out_id(char* name) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (pinv > 13) {
            ...
            if (output_pins[pinv - 5] == 0)
                ...
                value = lxT("          ") +
                        itoa(output_pins[pinv - 5]);  //+ lxT (" ") + Window5.GetPinName (output_pins[pinv - 5]);
        }
...
void cpart_IO_PCF8574::Process(void) {
    ...
    if (ioupdated) {
        ...
        if ((input_pins[0] > 0) && (input_pins[1] > 0))
            Window5.Set_pullup_bus(input_pins[1] - 1, io_PCF8574_I2C_io(&ioe8, ppins[input_pins[0] - 1].value,
