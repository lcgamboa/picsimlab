...
cpart_servo::~cpart_servo(void) {
    ...
    SpareParts.CanvasCmd({.partn = id, .cmd = CC_FREEBITMAP, .FreeBitmap{BackGround}});
...
void cpart_servo::DrawOutput(const unsigned int i) {
    if (Update == 1) {
        ...
        SpareParts.CanvasCmd({.partn = id,
                              .cmd = CC_SETBITMAP,
                              .SetBitmap{BackGround, 1.0, 1.0}});  // /FIXME draw servo error on scale or rotate
    ...
    switch (output[i].id) {
        ...
        case O_AXIS:
            ...
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{20}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_LINE, .Line{output[i].x1, output[i].y1, x2, y2}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETFGCOLOR, .SetFgColor{255, 255, 255}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_SETLINEWIDTH, .SetLineWidth{18}});
            SpareParts.CanvasCmd({.partn = id, .cmd = CC_LINE, .Line{output[i].x1, output[i].y1, x2, y2}});
            break;
    }
}
