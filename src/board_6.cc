...
{
  proc=P16F18855;  //default microcontroller if none defined in preferences
...
void 
cboard_6::RefreshStatus(_pic *pic)
{
   //verify serial port state and refresh status bar   
#ifndef _WIN_
    if(pic->serialfd > 0)
...
cboard_6::WritePreferences(void)
{
    ...
    Window1.saveprefs(wxT("p6_proc"),getnamebyproc(proc,line));
};
...
//Event on the board
void 
cboard_6::KeyPress(_pic *pic, uint key, uint x, uint y,uint mask)
{
...
//Event on the board
void
cboard_6::KeyRelease(_pic *pic, uint key, uint x, uint y,uint mask)
{
...
//Event on the board
void 
cboard_6::MouseButtonPress(_pic *pic, uint button, uint x, uint y,uint state)
{
...
//Event on the board
void 
cboard_6::MouseButtonRelease(_pic *pic, uint button, uint x, uint y,uint state)
{
  int i;
...
void cboard_6::Draw(_pic *pic, CDraw *draw,double scale)
{
  int i;
...
//board_6 draw 
    ...
    if(!output[i].r)//if output shape is a rectangle
    {
        ...
        switch(output[i].id)//search for color of output
        {
          ...
          case O_D2: //Red using pin 27 mean  value (RA0) 
            draw->Canvas.SetColor (lm[26], 0, 0);
            break;
          case O_D3: //Red using pin 28 mean  value (RA1) 
            draw->Canvas.SetColor (lm[27], 0, 0);
            break;
          case O_D4: //Red using pin 1 mean value (RA2)
            draw->Canvas.SetColor (lm[0], 0, 0);
            break;
  ...
  draw->Update ();
     ...
     gauge1->SetValue(0.4444*(lm[26]-30)); 
     //RA1 mean value to gauge2
     gauge2->SetValue(0.4444*(lm[27]-30)); 
     //RA2 mean value to gauge3
     gauge3->SetValue(0.4444*(lm[0]-30)); 
     //RC5 mean value to gauge4
     gauge4->SetValue(0.4444*(lm[1]-30)); 
   


};

void cboard_6::Run_CPU(_pic *pic)
{
  int i;  
  int j;
  unsigned char pi;
   ...
   for(i=0;i<Window1.GetNSTEP();i++) //repeat for number of steps in 100ms
      {
        ...
        {  
          ...
          pic_set_apin(pic,3,((5.0*(scroll1->GetPosition()))/
            (scroll1->GetRange()-1)));
